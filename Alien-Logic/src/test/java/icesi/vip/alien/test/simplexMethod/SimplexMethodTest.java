package icesi.vip.alien.test.simplexMethod;
...
import icesi.vip.alien.service.simplexAlgorithm.SensivilityAnalysis;
...
class SimplexMethodTest {
	...
	public void stepByStepTestNormal() {
		...
		double[][] expected = {{-3, -5, 0, 0, 0, 0},
				...
				{0, 2, 0, 1, 0, 12},
				{3, 2, 0, 0, 1, 18}};
		assertArrayEquals(expected, simplex.getActualMatrix());
	...
	public void sensitivityA1() {
		...
		assertArrayEquals(sA.getIntervalsDConstraints(), interDCons);
		assertArrayEquals(sA.getIntervalsDFO(), interDFO);
		assertArrayEquals(sA.getShadowPrice(), new double[][] {{1, 2, 0}});
	...
	public void sensitivityA2() {
		...
		assertArrayEquals(sA.getIntervalsDConstraints(), interDCons);
		assertArrayEquals(sA.getIntervalsDFO(), interDFO);
		assertArrayEquals(sA.getShadowPrice(), new double[][] {{11, 9}});
		double[] exp = simplex.getReducedCosts();
	...
	public void sensitivityA3() {
		...
		assertArrayEquals(sA.getIntervalsDConstraints(), simplex.roundMatrix(interDCons));
		assertArrayEquals(sA.getIntervalsDFO(), interDFO);
		assertArrayEquals(sA.getShadowPrice(), new double[][] {{0, 0.5, 1.5}});
		double[] exp = simplex.getReducedCosts();
	...
	public void sensitivityA4() {
		...
		assertArrayEquals(sA.getIntervalsDConstraints(), simplex.roundMatrix(interDCons));
		assertArrayEquals(sA.getIntervalsDFO(), simplex.roundMatrix(interDFO));
		assertArrayEquals(sA.getShadowPrice(), simplex.roundMatrix(new double[][] {{0.99159, 0, 0.43539, 0, 0, 0.07787, 0, 0, 0, -2.28672, 0}}));
		double[] exp = simplex.getReducedCosts();
